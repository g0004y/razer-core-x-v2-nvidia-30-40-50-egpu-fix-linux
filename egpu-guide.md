...
**The working fix for NVIDIA RTX 40-series eGPU crashes on Linux.** Complete guide to getting eGPU working via USB4/Thunderbolt — cold boot, hotplug, and suspend/resume.

---

## Background

I am not a Linux expert. I already owned the ThinkPad T14 Gen 3 and spent nearly **6 months** unable to use the eGPU on Linux after buying the RTX 4070 SUPER and Razer Core X V2. I tried countless kernel versions, driver versions, kernel parameters, and firmware tweaks. Nothing worked reliably. This guide is everything I learned — the hard way.
...
Hotplugging works **instantly**. The GPU is detected immediately after connecting the USB4 cable. `nvidia-smi` becomes available without any service restarts or manual intervention.
...
**This setup uses GNOME Wayland and it works perfectly.** Internal screen via PRIME offload, external monitor connected directly to the eGPU — both work reliably.
...
> **Full documentation, firmware files, and installation instructions are available in my [separate Bluetooth GitHub repository](https://github.com/g0004y/corsair-bluetooth-pairing-fix-intel-ax211-linux).**
...
**What happens:** After the Corsair firmware fix, the AX211 Bluetooth adapter still crashes when the Razer Core X V2 eGPU is connected and under sustained load. Kernel log shows:
...
**Why it happens:** The T14 Gen 3 uses **Intel AX211 CNVi** — Bluetooth is connected via internal USB. When the USB4 controller is carrying eGPU traffic, it consumes enough bandwidth to starve the Bluetooth chip's USB connection. This is a hardware-level bandwidth conflict.
...
**The fix — USB Bluetooth dongle:** The only reliable solution is a **USB Bluetooth dongle plugged directly into the laptop**.
...
- [egpu.io — Nvidia fallen off the bus (general)](https://egpu.io/forums/thunderbolt-linux-setup/nvidia-fallen-off-the-bus-and-is-not-responding-to-commands/)
- [egpu.io — ThinkPad T1g Gen 8 + Razer Core X V2 + RTX 4070](https://egpu.io/forums/thunderbolt-linux-setup/thinkpad-t1g-gen-8-core-x-v2-rtx-4070-gpu-has-fallen-off-the-bus/)
- [egpu.io — GPU fallen off bus after sleep](https://egpu.io/forums/thunderbolt-linux-setup/gpu-has-fallen-off-the-bus-error-19-after-resume-from-sleep/)
- [egpu.io — State of eGPU Hot-Plug for NVIDIA](https://egpu.io/forums/thunderbolt-linux-setup/state-of-egpu-hot-plug-for-nvidia/)
- [egpu.io — Razer Core X + RTX 4070 Super compatibility](https://egpu.io/forums/thunderbolt-enclosures/razer-core-x-not-chroma-not-compatible-with-new-rtx-4070-super/)
- [Bluetooth AX211 Firmware Fix (separate repo)](https://github.com/g0004y/corsair-bluetooth-pairing-fix-intel-ax211-linux)

---

## Why This Guide Exists

Getting an NVIDIA RTX 40-series eGPU working reliably on Linux via USB4/Thunderbolt has been a pain point for years. The egpu.io forums are full of people struggling with the same errors:

- `NVRM: Xid 79, GPU has fallen off the bus`
- `NVRM: GPU has fallen off the bus and is not responding to commands`
- eGPU detected by `lspci` but not by `nvidia-smi`
- Random initialization failures on boot
- Hotplug not working

Information was scattered across random Reddit threads, forum comments, and half-finished GitHub issues — none with a complete working solution. NVIDIA's own documentation is technically detailed but complex and hard to apply to real-world eGPU setups. Most guides covered one piece of the puzzle — kernel params, driver versions, Thunderbolt authorization — but never explained how they fit together.

This guide puts everything under one roof: root cause, the exact fix, why it works, what was tried and why it failed, and the technical details behind each decision. It documents 6 months of troubleshooting so you don't have to repeat it.

---

## Disclaimer

This guide documents exactly what worked for my setup. Your hardware combination, BIOS settings, USB4 implementation, and kernel driver versions may differ — results can vary. The core fix (disabling GSP) is generally applicable to RTX 40-series eGPUs on Linux, but the specific kernel and driver version requirements are specific to this hardware.

Everything in this guide is explained in detail — root cause, why the fix works, and why other approaches fail. Read the "Why It Happens" and "Why This Works" sections before applying anything, so you understand what each step does and how to adapt it if needed.
